# Desenvolvimento-Front-End-Para-Web-2025

## Objetivo do projeto

Criar um sistema web simples para gerenciar alunos: permitir cadastrar, visualizar, editar e remover registros de alunos de maneira intuitiva. O projeto serve como exercício prático de front-end (HTML, CSS e JavaScript) e utiliza armazenamento local do navegador (localStorage) para persistência dos dados durante testes.

## Funcionalidades

- Cadastro de novos alunos com validação básica do formulário.
- Exibição da lista de alunos cadastrados.
- Edição e remoção de alunos da lista.
- Navegação por menu interno entre telas/abas (por exemplo: cadastro e lista).
- Persistência dos dados no navegador usando localStorage (sem back-end).

## Tecnologias utilizadas

- HTML
- CSS
- JavaScript (Vanilla)

## Instruções para execução

1. Clone ou baixe o repositório para sua máquina.
2. Abra o arquivo `index.html` no navegador (duplo clique ou via servidor local).
3. Recomenda-se usar um navegador moderno (Chrome, Firefox, Edge).
4. Os dados são salvos no localStorage do navegador; limpar o cache/armazenamento remove os registros.

## Estrutura de pastas sugerida

projeto/
├─ index.html
├─ css/
│  └─ style.css
└─ js/
   └─ script.js

## Observações

- Este projeto foi pensado para fins didáticos; não há back-end nem autenticação.
- Se quiser integrar com um servidor, posso ajudar a adaptar o código.